...
### Changed
- `examples/6_advanced_rendering.py --task libero.pick_butter --demo N` renders a registered task's recorded demonstration: the task scenario replaces the primitive scene, MuJoCo replays the demo's actions through `HybridSimHandler.simulate_steps(actions=...)`, and the manifest records the trajectory file hash, demo index and action count; `--camera-pos` / `--look-at` frame the workspace.
- `examples/6_advanced_rendering.py` can render inside any `roboverse_pack` interior scene (`--background kujiale_scene_0009`, dropping the primitive floor/backdrop while MuJoCo keeps a flat ground) and draws HDRIs from a directory (`--hdri-library DIR`); the scene file joins the manifest's `scene_asset_sha256`.
- `roboverse_pack/scenes` configs now resolve a `blender` file type (USD), so Blender can load the Kujiale / manycore / Arnold scenes instead of failing with `KeyError`.
- `examples/6_advanced_rendering.py` renders textured floors and backdrops from a `--material-library` of `*_BaseColor/_Normal/_ORM.png` sets (metric box UVs), Kelvin key/fill lights, depth of field and the image-space sensor stage by default (`--no-depth-of-field`, `--no-sensor`). It keeps raw and processed first frames, writes per-frame render and sensor calibration, and records scene asset hashes and renderer versions in the manifest.
- Advanced rendering defaults to a studio appearance preset with explicit collision/render support geometry and key/fill lighting. Videos batch intermediate physics intervals and default to the actual physics capture rate (`--fps` overrides playback); manifests include both time axes and image diagnostics. Replay verifies existing recipe and texture/HDRI hashes before launch, with an explicit opt-out for intentional edits.
- The parity / consistency probes moved from `scripts/` to `tools/parity/` (import path `tools.parity.*`) and the asset helpers to `tools/assets/`; `scripts/` keeps the developer scripts (data collection, teleop, replay, conversion, mesh tools, OSC, Unitree deployment).
...
### Fixed
- The demo converters (`roboverse_learn/il/data2zarr_dp.py`, the pi0 and SmolVLA `convert_roboverse_to_lerobot.py`) refuse a demo they cannot convert before reading any frame, with the reason (`roboverse_learn/il/utils/demo_metadata.py`; the LeRobot converters, which run inside the policy environment where that package is not importable, carry a local null-target check and keep their warn-and-truncate on a length mismatch): the zarr converter's `ee` spaces read `robot_ee_state` / `robot_ee_state_target` as `[pos, quat, ...]`, which no writer produces (`save_demo` records `ee_state` as `[pos, rpy, gripper]` and no EE target), so they always failed with a `KeyError` in the frame loop; a `null` or misaligned `joint_qpos_target` (a backend that reports no targets) was a `TypeError` on slicing in all three.
- `scripts/advanced/collect_demo.py`: a demo that succeeded on the same step its episode timed out was counted as a success and then saved as a failure; the outcome rule is one tested function (`demo_outcomes` in `roboverse_learn/il/utils/clean_state.py`) and each env lands in at most one save set. `roboverse_learn/managers/manager_based_rv_env.py` wrapped every command-manager and sensor update / reset in `except Exception: pass`, so a broken sensor fed zeroed contact history into the reward silently; they raise, and the mjlab sensors that need Newton's contact / subtree queries decide their backend at construction, with the DR events' backend naming and `NotImplementedError`: on `--sim mujoco --num_envs > 1` (the parallel wrapper) and on MJX the handler exposes neither, so the velocity tasks' contact sensors failed on the first update behind that swallow; the terrain sensors, whose Newton path reads tensor states only, run on any backend. The same swallow one frame down (`mdp/commands.py` heading read, `mdp/curriculums.py` command read) is removed too. `act_eval_runner.py` paired the reindexed action with the config's joint-limit key order (correct for the bundled Franka, whose config lists joints in MJCF order, but dependent on that ordering); it now pairs the policy's sorted-order output with the sorted joint names, with no reindex to keep consistent.
- FastTD3 `evaluate.py` / `evaluate_lift.py` wrote the *achieved* `joint_pos` after each step under `dof_pos_target` in the v2 trajectories they collect, so the recorded action lagged the real command by the controller's tracking error and a replay drove the robot to where it had already been. They record the `joint_pos_target` the handler reports (Genesis and Isaac Gym now derive it from the applied action, so every position-controlled backend reports it), keep the terminal (action, state) pair of every episode (from `info["terminal_states"]` for an auto-reset env, and whatever the sampling stride) instead of the reset pose, raise on the first step that cannot be recorded instead of evaluating with nothing saved, let a done env start its next episode from the state `RLTaskEnv.step` auto-reset it to (an env a task marks done after that base step, as the pick-place release check does, is reset by the evaluator) instead of resetting every env whenever one finished (which spliced the other envs' partial episodes onto their next one and carried their partial returns into the next episode's statistics), key joints by the handler's order for robots and articulated objects alike, and build the state dicts with `metasim.utils.state.state_tensor_to_nested` (`roboverse_learn/rl/fast_td3/trajectory_record.py`) instead of two private copies.
- Evaluation numbers: `BaseEvalRunner.get_temporal_agg_action` (the ACT-style ensembling every IL policy runner uses) weighted the *newest* chunk most (`exp(+k i)`) where ACT weights the oldest most (`exp(-k i)`), and selected the overlapping chunks by a non-zero test on action values ANDed across envs, so a legitimately zero action in one env dropped that chunk for every env; chunks are now selected by position, in `BaseEvalRunner` and in `act_eval_runner.py`'s inline copy (the vendored ACT training script `imitate_episodes.py` keeps ACT's original selection). `act_eval_runner.py` scored env 0 only while dividing by the episode count and capped episodes at a hard-coded 800 steps unrelated to its buffers; it now refuses `--num_envs != 1` (it drives one env) and runs to the task's episode length. The three VLA evaluators refuse `--num_episodes < 1` instead of dividing by zero or printing 0 %.

- beyondmimic's `ContactForces` query reuses MetaSim's `mujoco_net_contact_forces_world` instead of a forked copy of the MuJoCo branch: contact forces on MuJoCo are now world-frame forces on the body (the fork summed contact-frame components with the reaction sign), so beyondmimic contact rewards / terminations on MuJoCo change accordingly and agree with `metasim.queries.ContactForces`.
- mjlab tasks: the reset noise of `velocity_go1_v2`, `velocity_g1_v2`, `lift_cube_yam_v2`, `cartpole_train` and the object-pose command in `mdp/commands` now draws from the RNG that `handler.set_seed` seeds, so `reset(seed=N)` reproduces them; they built a fresh `np.random.default_rng()`, which ignores the seed (the `cartpole_v2` fix, applied to the family). The three `mdp/events_dr` samplers are fixed the same way, but they run as setup events at env construction, before any `reset(seed=)`: seed the process (`np.random.seed`) before building the env to reproduce them. `tests/test_task_reset_seed_contract.py` now rejects any unseeded NumPy generator under `roboverse_pack/tasks`, and `tests/test_mjlab_dr_backend_support.py` checks the DR draws are reproducible through `set_seed`.
- `scripts/advanced/collect_demo.py`: with `num_envs > 1`, resetting one finished env settled the whole batch (`ensure_clean_state` steps every env), so every other in-flight demo absorbed physics steps that were never recorded; the saved trajectories skipped motion and did not replay (a velocity jump with no action change, found by replaying an `episode.npz` in a fresh process). Those settle steps are now appended to the demos of the envs that keep stepping (envs whose demo ends in the same iteration are excluded until they are reset and recording again, so a closed demo never gains trailing frames and a fresh one never loses its first steps) with the joint target they were holding. `ensure_clean_state` gains an `on_step` callback for this; `--traj_filepath` overrides a task's demo file (honoured whether the task declares it on the class or assigns it on the instance, without mutating the registered class) and a demo-free task is reported instead of `os.path.exists(None)`.
- VLA evaluation (`OpenVLA/vla_eval.py`, `SmolVLA/smolvla_eval.py`, `pi0/pi_eval.py`) imported `ensure_clean_state` from `roboverse_learn.il.act`, a path that no longer exists, so every run with domain randomization and initial states crashed at the first reset. The helper (three byte-identical copies in the IL runner, the ACT evaluator and `collect_demo.py`) now lives once in `roboverse_learn/il/utils/clean_state.py` and actually works: the settle check compared a field that does not exist (`dof_pos` instead of `joint_pos`) and always passed after four steps, and the initial-state validation could not read the demo dicts every caller passes, so it never fired. It now settles on object joint positions and object positions (robots are excluded: an unheld arm sags every step and the policy takes it over), warns when the scene has not settled within `max_steps`, and compares the settled object joints of the reset env (`env_id`) with the demo's `dof_pos`.
- `tests/test_learn_lazy_imports.py` resolves every first-party import (`roboverse_learn`, `roboverse_pack`, `metasim` targets across `roboverse_learn/`, `roboverse_pack/`, `scripts/`, `examples/`, `tools/`) on disk without importing anything and checks the imported names are defined, so a moved module or renamed function fails in CI instead of in someone's evaluation. It caught two more stale imports: `roboverse_pack/asset/table_display.py` (module moved to `roboverse_pack.asset.table785_config`) and `tools/dashboard/minimal_test_script.py` (`get_sim_env_class` is `get_sim_handler_class`; only the import is fixed, the script still targets a pre-handler task API and is not runnable).
- The ACT docs page points at `roboverse_learn/il/policies/act/` again (`il/act/` no longer exists).
...
- Extended `examples/6_advanced_rendering.py` with an asset-free primitive default, optional assets/robot, Blender / Isaac Sim rendering, MuJoCo physics, deterministic recipe export/replay and sharding, multi-view video, calibration, asset hashes and atomic per-variant manifests. The previous asset-heavy scene is opt-in via `--scene assets --robot franka`. Updated rendering and randomization documentation with backend capabilities and validation limits.

- `tests/test_examples_smoke.py`: the nine getting-started examples run headless on MuJoCo as subprocesses and must exit 0 without a traceback, including at interpreter exit. Hosted CI disables MuJoCo rendering (`MUJOCO_GL=disable`), where the lane skips with that reason; elsewhere the examples run with `MUJOCO_GL=egl`, so the lane runs on any machine with EGL. The two examples that never closed their handler now do.
