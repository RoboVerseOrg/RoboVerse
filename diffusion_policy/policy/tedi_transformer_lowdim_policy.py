...
#from diffusion_policy.model.diffusion.transformer_for_diffusion_flash import TransformerForDiffusionFlash
...
class TEDiTransformerLowdimPolicy(BaseLowdimPolicy):
    def __init__(
        self,
        ...
        **kwargs,
        ...
        if self.action_buffer is None:
            trajectory = torch.randn(
                size=shape, dtype=condition_data.dtype, device=self.device, generator=generator
            )
            ...
            for t in scheduler.timesteps.to(device=self.device, dtype=torch.long):
                ...
                trajectory = scheduler.step(
                    ...
                    generator=generator,
                    **kwargs,
                ).prev_sample.to(condition_data.dtype)
    ...
    def compute_loss(self, batch):
        ...
        pred = self.model(
            noisy_trajectory, diff_steps, cond
        )
