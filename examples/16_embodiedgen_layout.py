...
from tools.embodiedgen_integration.enums import AssetType, SimAssetMapper
from tools.embodiedgen_integration.load_asset import load_embodiedgen_asset, load_embodiedgen_layout_pose
...
if __name__ == "__main__":
    ...
    # First frame image
    save_path = f"examples/output/16_embodiedgen_layout_{args.sim}.png"
    log.info(f"Saving image to {save_path}")
    obs = handler.get_states(mode="dict")[0]
    imageio.imwrite(save_path, obs["cameras"]["camera"]["rgb"])

    # Video
    obs_saver = ObsSaver(video_path=f"examples/output/16_embodiedgen_layout_dynamic_{args.sim}.mp4")
    total_step = 100
