...
```bash
python examples/viser/viser_demo.py --sim mujoco
```

**With Joint Control**:
```bash
python examples/viser/viser_demo.py --sim mujoco --enable-joint-control
```

**With IK Control**:
```bash
python examples/viser/viser_demo.py --sim mujoco --enable-ik-control
```

**With Trajectory Playback**:
```bash
python examples/viser/viser_demo.py --sim mujoco --enable-trajectory
```

**Dynamic Simulation** (robot moves with IK):
```bash
python examples/viser/viser_demo.py --sim mujoco --dynamic
```

**Dynamic with Video Recording**:
```bash
python examples/viser/viser_demo.py --sim mujoco --dynamic --save-video
```
