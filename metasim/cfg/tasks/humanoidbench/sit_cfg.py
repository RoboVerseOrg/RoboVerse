...
class SitReward(HumanoidBaseReward):
    ...
    def __call__(self, states: dict) -> torch.FloatTensor:
        ...
        # 2) Posture & upright
        # ------------------------------------------------------------------
        sitting_posture = tolerance_tensor(
            head_z - imu_z,
            bounds=(0.35, 0.45),
            margin=0.3
        )
        ...
        # ------------------------------------------------------------------
        dont_move = tolerance_tensor(torch.stack([vx, vy], dim=-1),  # (B, 2)
                            margin=2.0).mean(dim=-1)        # → (B,)
