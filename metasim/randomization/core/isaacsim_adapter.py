...
class IsaacSimAdapter:
    ...
    def __init__(self, handler: IsaacsimHandler):
        ...
        try:
            ...
            raise ImportError(f"IsaacSim modules not available: {e}")
    ...
    def set_transform(
        ...
        if rotation is not None:
            ...
            orient_ops = [
                op for op in xform_ops 
                if op.GetOpType() == self.UsdGeom.XformOp.TypeOrient
            ]
        ...
        if scale is not None:
            scale_ops = [
                op for op in xform_ops 
                if op.GetOpType() == self.UsdGeom.XformOp.TypeScale
            ]
            if scale_ops:
    ...
    def get_transform(self, prim_path: str) -> tuple[tuple, tuple, tuple]:
        ...
        for op in xform_ops:
            if op.GetOpType() == self.UsdGeom.XformOp.TypeTranslate:
                ...
                quat = op.Get()
                rotation = (quat.GetReal(), quat.GetImaginary()[0], 
    ...
    def apply_mdl_material(
        ...
        if not success:
            raise RuntimeError(f"Failed to bind material {mtl_name} to {prim_path}")
    ...
    def apply_pbr_material(self, prim_path: str, pbr_config: dict):
        ...
        if "roughness" in pbr_config:
            shader.CreateInput("roughness", self.Sdf.ValueTypeNames.Float).Set(
                float(pbr_config["roughness"])
            )
        
        if "metallic" in pbr_config:
            shader.CreateInput("metallic", self.Sdf.ValueTypeNames.Float).Set(
                float(pbr_config["metallic"])
            )
        
        if "specular" in pbr_config:
            shader.CreateInput("specular", self.Sdf.ValueTypeNames.Float).Set(
                float(pbr_config["specular"])
            )
