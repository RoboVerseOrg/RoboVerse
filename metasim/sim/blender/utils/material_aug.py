...
Targets the open6dor ``material_lib_v2.blend`` (165 wood/rubber/metal/plastic/
background materials), but works with any .blend file as long as you point
``library_path`` at it.

The library file is large (~1.5 GB) — we only ``append`` the specific material
we want to assign, on demand, and cache the result so repeated picks don't
re-import. A small in-memory bookkeeping dict tracks already-appended names
so we don't accumulate duplicates across many frames.
"""
...
def make_random_pbr_material(
    ...
    Useful when you don't have ``material_lib_v2.blend`` handy. Covers the
    plastic/metal/rubber spectrum by sweeping metallic + roughness + colour.
    """
    rng = rng or random
    ...
    bsdf.inputs["Metallic"].default_value = rng.choice([0.0, 0.0, 1.0])  # weight toward dielectric
