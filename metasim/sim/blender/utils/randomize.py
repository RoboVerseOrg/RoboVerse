"""Sampling helpers for camera / object pose / robot DOF augmentation."""
...
class HemisphereCfg:
    ...
    # Small jitter on the look_at target so the subject isn't dead-centred.
...
def disjoint_xy_samples(
    ...
    for _ in range(n):
        for _try in range(max_tries):
            ...
            ok = all(math.hypot(x - px, y - py) >= min_dist for px, py in pts)
            if ok:
                pts.append((x, y))
                break
        else:
            pts.append((x, y))  # accept last attempt to keep length stable
    return pts
