...
class IKSolver:
    ...
    def solve_ik_batch(self, ee_pos_target: torch.Tensor, ee_quat_target: torch.Tensor, 
    ...
    def compose_full_joint_command(self, q_solution: torch.Tensor, gripper_widths: torch.Tensor,
                                 current_q: torch.Tensor | None = None) -> torch.Tensor:
        ...
        q_full = current_q[:, :self.n_robot_dof].clone() if current_q is not None else \
                 torch.zeros((num_envs, self.n_robot_dof), device=device)
...
def setup_ik_solver(robot_cfg: RobotCfg, solver: Literal["curobo", "pyroki"] = "pyroki", 
                   no_gnd: bool = False) -> IKSolver:
    """Setup IK solver."""
    return IKSolver(robot_cfg, solver, no_gnd)


def process_gripper_command(gripper_binary: torch.Tensor, robot_cfg: RobotCfg, device: torch.device | str) -> torch.Tensor:
