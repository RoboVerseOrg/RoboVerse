...
def list_state_to_tensor(
    ...
    for name in obj_names:
        ...
        root, body, jpos, jvel = _alloc_state_tensors(
            n_env, len(bnames) or None, len(jnames) or None, dev
        )
        ...
        objects[name] = ObjectState(
            root_state=root, body_state=body,
            ...
            if body is not None and "body" in s:
                for i, bn in enumerate(sorted(bnames)):
                    ...
                    body[e,i,:3], body[e,i,3:7], body[e,i,7:10], body[e,i,10:13] = \
                        bi["pos"], bi["rot"], bi["vel"], bi["ang_vel"]

        robots[name] = RobotState(
            root_state=root,
            body_names=bnames, body_state=body,
            joint_pos=jpos, joint_vel=jvel,
            joint_pos_target=jpos_t, joint_vel_target=jvel_t,
            joint_effort_target=jeff_t,
        )

    # -------- cameras ---------------------------------------------
    for cam in cam_names:
        rgb   = torch.stack([es["cameras"][cam]["rgb"]   for es in env_states if "cameras" in es and cam in es["cameras"]], dim=0).to(dev)
        depth = torch.stack([es["cameras"][cam]["depth"] for es in env_states if "cameras" in es and cam in es["cameras"]], dim=0).to(dev)
