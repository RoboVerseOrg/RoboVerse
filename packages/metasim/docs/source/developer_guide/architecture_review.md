...
| ``set_states`` | dict has ``vel`` / ``ang_vel`` / ``dof_vel`` | Restored where the backend declares ``set_states_restores_velocities`` (MuJoCo, SuperDex, Newton, Isaac Sim, Isaac Gym on the ``TensorState`` path); dropped with a one-shot warning on MJX (unless ``zero_vel=False``), SAPIEN 3, PyBullet, Genesis. | Check the flag; on a dropping backend initialise momentum via ``simulate()`` with a non-zero initial action. |
