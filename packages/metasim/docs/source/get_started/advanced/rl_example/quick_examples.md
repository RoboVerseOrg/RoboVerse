...
python examples/rl/0_ppo.py --task reach_origin --robot franka --sim isaacgym --num-envs 128

# Train with MuJoCo backend
python examples/rl/0_ppo.py --sim mujoco --headless

# Use Gym interface for cleaner integration
python examples/rl/0_ppo_gym.py --sim mjx --num-envs 256
```
...
python examples/rl/0_ppo.py --sim isaacgym --headless

# Isaac Lab (next-generation)
python examples/rl/0_ppo.py --sim isaaclab --headless

# MuJoCo (CPU/GPU)
python examples/rl/0_ppo.py --sim mujoco --headless

# Genesis (multi-physics)
python examples/rl/0_ppo.py --sim genesis --headless

# MJX (JAX-based, fastest)
python examples/rl/0_ppo_gym.py --sim mjx --headless
```
