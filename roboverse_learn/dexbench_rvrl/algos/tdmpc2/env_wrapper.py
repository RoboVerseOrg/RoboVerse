...
class Tdmpc2DexEnv:
    ...
    def __init__(self, obs_shape: dict, num_frames: int, env: DexEnv, device: str):
        ...
        for key in obs_shape.keys():
            if "rgb" in key:
                ...
                self._frames_tensor_obs[key] = torch.zeros((env.num_envs, num_frames, *obs_shape[key]), dtype=torch.float32, device=self.device)
    ...
    def reset(self):
        ...
        self.tensor_obs = self.env.tensor_obs
        for key, value in observations.items():
            if "rgb" in key:
                for i in range(self._num_frames):
                    self._frames[key][:, i] = value.to(self.device).clone()
                    self._frames_tensor_obs[key][:, i] = self.tensor_obs[key].to(self.device).clone()
                observations[key] = self._frames[key].reshape(self.num_envs, self._num_frames * self.obs_shape[key][0], *self.obs_shape[key][1:])
                self.tensor_obs[key] = self._frames_tensor_obs[key].reshape(self.num_envs, self._num_frames * self.obs_shape[key][0], *self.obs_shape[key][1:])
        return observations
    ...
    def step(
        ...
        for key, value in observations.items():
            ...
            if "rgb" in key:
                ...
                cv2.imwrite("tdmpc2_image.png", img_bgr)
                # exit(0)
                if dones.any():
                    for i in range(self._num_frames):
                        self._frames[key][dones, i, ...] = value[dones].to(self.device).clone()
                        self._frames_tensor_obs[key][dones, i, ...] = self.tensor_obs[key][dones].to(self.device).clone()
                    self._frames[key][~dones, :-1, ...] = self._frames[key][~dones, 1:, ...].clone()
                    self._frames[key][~dones, -1, ...] = value[~dones].to(self.device).clone()
                    self._frames_tensor_obs[key][~dones, :-1, ...] = self._frames_tensor_obs[key][~dones, 1:, ...].clone()
                    self._frames_tensor_obs[key][~dones, -1, ...] = self.tensor_obs[key][~dones].to(self.device).clone()
                else:
                    self._frames[key][:, :-1] = self._frames[key][:, 1:].clone()
                    self._frames[key][:, -1] = value.to(self.device).clone()
                    self._frames_tensor_obs[key][:, :-1] = self._frames_tensor_obs[key][:, 1:].clone()
                    self._frames_tensor_obs[key][:, -1] = self.tensor_obs[key].to(self.device).clone()
                self.tensor_obs[key] = self._frames_tensor_obs[key].reshape(self.num_envs, self._num_frames * self.obs_shape[key][0], *self.obs_shape[key][1:])
                observations[key] = self._frames[key].reshape(self.num_envs, self._num_frames * self.obs_shape[key][0], *self.obs_shape[key][1:])
