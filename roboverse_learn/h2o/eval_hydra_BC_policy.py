...
def play(cfg_hydra: DictConfig) -> None:
    ...
    eval_runner = EvalRunnerBCModified(
        env=env, policy=policy, train_cfg=train_cfg, device=env.device, To=To, clip_action=clip_action
    )

    results = eval_runner.eval()
    ckpt_path = (
        cfg_hydra.BC_ckpt_path
    )  # 例如 "/home/yunshen/code/test_ckpt/amass_200k_mix@amass_100k_easy_clean/epoch_400_step_663387.ckpt"
