...
import xformers
from xformers.components import TransformerEncoder, TransformerDecoder
from xformers.factory import xFormerEncoder, xFormerDecoder
logger = logging.getLogger(__name__)

class TransformerForDiffusionFlash(ModuleAttrMixin):
    def __init__(self,
            input_dim: int,
            output_dim: int,
            horizon: int,
        ...
        if T_cond > 0:
            ...
            if n_cond_layers > 0:
                encoder_layer = xFormerEncoder(
                    n_head=n_head,
                    dim_model=n_emb,
                    dim_feedforward=4*n_emb,
                    dropout=p_drop_attn,
                    activation="gelu",
                    attention_type="flash"  # 指定使用 Flash Attention
                )
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, TransformerForDiffusionFlash):
