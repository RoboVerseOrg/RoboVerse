...
class BaseCalvinTableTask(BaseTaskEnv):
        ...
        def compare_entity(e1, e2):
            ...
            if 'dof_pos' in e1:
                ...
                for joint_name, val1 in dof1.items():
                    ...
                    if isinstance(val1, torch.Tensor): val1 = val1.item()
                    if isinstance(val2, torch.Tensor): val2 = val2.item()
    ...
    def step(self, action):
        ...
        if self.scenario.robots[0].control_type == "joint_position":
            ...
            success = self.check_state_tolerance(obs)
            ...
            obs, reward, success, time_out, extras = super(BaseCalvinTableTask).step(actions)
            success = self.check_state_tolerance(obs)
    ...
    def check_state_tolerance(self, current_state,
                              pos_tol=0.05,
                              rot_tol=0.1,
                              joint_tol=0.1,
                              verbose=False):
