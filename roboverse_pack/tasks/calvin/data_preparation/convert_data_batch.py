...
def update_env_state_from_dataset_obs(
    current_env_state_list, 
    ...
    for obs_name, state_name in BLOCK_MAP.items():
        if state_name in state_to_update['objects']:
            ...
            quat_wxyz = quat_from_euler_np(block_data['rot_euler'][0],block_data['rot_euler'][1],block_data['rot_euler'][2])
    ...
    for env_files in file_list:
        ...
        for file in files:
            ...
            for (start, end) in segments:
                ...
                init_state = env.handler.get_states(mode = 'dict')
