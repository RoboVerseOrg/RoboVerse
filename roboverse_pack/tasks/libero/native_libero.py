...
from roboverse_pack.tasks.robosuite._osc import NativeOSC  # noqa: E402
from roboverse_pack.tasks.robosuite.robosuite_env import _GroundlessMujocoHandler  # noqa: E402

from ._native_util import check_bddl_success, remap_libero_model  # noqa: E402
...
class NativeLiberoEnv(BaseTaskEnv):
    ...
    def __init__(self, scenario: ScenarioCfg | None = None, device: str | torch.device | None = None):
        ...
        xml = remap_libero_model(model_xml, _libero_assets())
        ...
        self._decimation = int(round((1.0 / self.control_freq) / 0.002))
        sc = ScenarioCfg(
            scene=SceneCfg(mjcf_path=self._xml_file.name), robots=[], lights=[], ground=None,
            sim_params=SimParamCfg(dt=0.002), decimation=self._decimation,
    ...
    def _setup_native(self):
        ...
        self._grip_act = [a for a in (aid("gripper0_gripper_finger_joint1"), aid("gripper0_gripper_finger_joint2")) if a >= 0]
    ...
    def _make_osc(self):
        self._osc = NativeOSC(
            self._m, self._d, eef_site="gripper0_grip_site",
            ...
            initial_joint=_PANDA_INIT, base_pos=self._base_pos, base_ori=self._base_ori,
        )
