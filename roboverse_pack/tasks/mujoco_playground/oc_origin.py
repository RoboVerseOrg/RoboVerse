...
class PandaOpenCabinet(panda.PandaBase):
    ...
    def __init__(
        ...
        xml_path = mjx_env.ROOT_PATH / "manipulation" / "franka_emika_panda" / "xmls" / "mjx_cabinet.xml"
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        target_pos = target_pos.at[0].set(target_pos[0] + jax.random.uniform(rng_target, minval=-0.1, maxval=0.1))
        ...
        init_ctrl = jp.array(self._init_ctrl).at[:7].set(self._init_ctrl[:7] + perturb_arm)
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        data: mjx.Data = mjx_env.step(self._mjx_model, state.data, ctrl, self.n_substeps)
    ...
    def _get_rewards(self, data: mjx.Data, info: dict):
        ...
        robot_target_qpos = 1 - jp.tanh(
            jp.linalg.norm(data.qpos[self._robot_arm_qposadr] - self._init_q[self._robot_arm_qposadr])
        )

        # Check for collisions with the barrier
        hand_barrier_collision = [
            data.sensordata[self._mj_model.sensor_adr[sensor_id]] > 0 for sensor_id in self._barrier_hand_found_sensor
        ]
