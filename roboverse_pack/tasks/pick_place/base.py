...
class TrajectoryTrackingTaskBase(RLTaskEnv):
    ...
    def __init__(self, scenario, device=None):
        ...
        self.gripper_open_q = torch.tensor(
            robot_config.gripper_open_q, device=self.device
        )  # (11,) finger open positions
        self.gripper_close_q = torch.tensor(
    ...
    def step(self, actions):
        ...
        # Calculate how closed the hand is (0 = open, 1 = closed)
        # Formula: ratio = (current - open) / (close - open)
        # When current = open: ratio = 0 (fully open)
